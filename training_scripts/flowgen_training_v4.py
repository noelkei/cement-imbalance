...
"""
flowgen_training_v90.py

Runs the 90-config plan (Block A: 30 extreme-W1 configs with w1x = 10*w1y;
Block B: 60 best/moderate configs) for each finetune epoch bucket in --finetune_epochs_list.
Default is "30,50". Prefix is FT90_{finetune_epochs}.

Key choices:
- class_weighting = uniform everywhere
- ref_min == syn_min (swept together in a small block)
- realism_ramp_epochs = 0 by default; a few guarded variants use 4
- KS off by default; some variants enable light KS with unique (weight, grid, tau)
- MMD_x around {0.48, 0.52, 0.58, 0.62}; MMD_y in {1.0, 1.25, 1.5, 1.75}
"""
...
STATE_PATH = SWEEP_ROOT / "flowgen_training_v90_state.json"
...
# -------------------------
# Spec builder (90 runs)
# -------------------------
def build_90_specs(finetune_epochs: int) -> List[Dict[str, Any]]:
    specs: List[Dict[str, Any]] = []
    common = common_finetune_overrides(finetune_epochs)

    # Baseline anchor (diff-only) — use a dict literal (not dict(...)) to allow quoted keys
    base = {
        # W1
    ...
    # ---------- Block A (30) Extreme W1 with w1x = 10 * w1y
    levels = [(2, 20), (3, 30), (4, 40), (5, 50), (7, 70), (10, 100)]
    for li, (wy, wx) in enumerate(levels, start=0):
        # 1) raw
        add(f"A{li*5+1:02d}", training__w1_y_weight=float(wy))  # placeholder for lint; will be overwritten below
        # Replace the placeholder by adding a full row (kept as two lines here for readability)
        specs.pop()
        ...
        # 3) + slight MMD tweak
        mmdx = 0.48 if wy >= 7 else 0.45
        add(f"A{li*5+3:02d}",
            **{
                "training.w1_y_weight": float(wy), "training.w1_x_weight": float(wx),
                "training.mmd_x_weight": mmdx, "training.mmd_y_weight": 1.25,
            })
        ...
        add(f"A{li*5+4:02d}",
            **{
                ...
                "training.ks_grid_points_y": 80, "training.ks_tau_y": 0.045,
            })

        # 5) + light KS-X&Y (guards for largest weights)
        extra = {}
        if wy >= 7:
            extra = {"training.realism_ramp_epochs": 4, "training.realism_z_trunc": 2.5}
        add(f"A{li*5+5:02d}",
            **{
                "training.w1_y_weight": float(wy), "training.w1_x_weight": float(wx),
                "training.use_ks_x": True, "training.ks_x_weight": 0.007,
                "training.ks_grid_points_x": 80, "training.ks_tau_x": 0.045,
                "training.use_ks_y": True, "training.ks_y_weight": 0.015,
                "training.ks_grid_points_y": 80, "training.ks_tau_y": 0.045,
                **extra,
            })

    # ---------- Block B (60)
    # B01–B16: MMD grid, no KS
    mmdx_vals = [0.48, 0.52, 0.58, 0.62]
    mmdy_vals = [1.0, 1.25, 1.5, 1.75]
    idx = 1
    for mx in mmdx_vals:
        for my in mmdy_vals:
            add(f"B{idx:02d}", **{"training.mmd_x_weight": mx, "training.mmd_y_weight": my})
            idx += 1  # ends at 16

    # B17–B24: tiny KS-Y on selected MMDs
    b_ks_y = [
    ...
    for (mx, my, ky, gy) in b_ks_y:
        add(f"B{idx:02d}",
            **{
                ...
                "training.ks_grid_points_y": gy, "training.ks_tau_y": 0.045,
            })
        idx += 1  # to 24

    # B25–B36: gentle KS on both X and Y
    b_both = [
    ...
    for (mx, my, kx, gx, tx, ky, gy, ty) in b_both:
        add(f"B{idx:02d}",
            **{
                ...
                "training.use_ks_y": True, "training.ks_y_weight": ky,
                "training.ks_grid_points_y": gy, "training.ks_tau_y": ty,
            })
        idx += 1  # to 36

    # B37–B44: moderate W1 pushes
    b_w1 = [
    ...
    # B53–B56: equal sampling min sweeps
    for rr, ss, mx, my in [(200, 200, 0.58, 1.50),
                           (300, 300, 0.52, 1.50),
                           (400, 400, 0.52, 1.25),
                           (500, 500, 0.58, 1.25)]:
        add(f"B{idx:02d}",
            **{"training.ref_min": rr, "training.syn_min": ss,
               "training.mmd_x_weight": mx, "training.mmd_y_weight": my})
        idx += 1

    # B57–B60: small stability toggles
    add(f"B{idx:02d}",
...
# -------------------------
def main():
    ap = argparse.ArgumentParser(description="FlowGen finetune-only sweep (90 combos).")
    ...
    ap.add_argument("--finetune_epochs_list", type=str, default="30,50",
    ...
    for fe in ft_buckets:
        specs = build_90_specs(finetune_epochs=fe)
    ...
    print(f"🧪 Planned total: {len(ft_buckets)}×90 = {len(ft_buckets)*90}")
    ...
    try:
        while pending:
            ...
            for idx, (spec, base_name) in enumerate(pending, start=1):
                ...
                fe_match = re.search(r"FT90_(\d+)_", base_name)
    ...
    summary_csv = SWEEP_ROOT / f"sweep_flowgen_finetune_v90_{ts}.csv"
