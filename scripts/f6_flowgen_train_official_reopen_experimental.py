...
OUTPUT_SUBDIR = "reopen_experimental"
MODEL_FAMILY = "flowgen"
CONTRACT_ID = "f6_flowgen_official_reopen_experimental_v1"
OBJECTIVE_METRIC_ID = "flowgen_realism_official_reopen_experimental"
...
SUMMARY_ROOT = OFFICIAL_FLOWGEN_ROOT / "campaign_summaries" / "reopen_experimental"
...
PREFERRED_POLICY_ORDER = [
    "O01_now1x_mmdheavy",
    "O02_now1x_mmdheavy_mmdxy",
    "O03_now1x_ksx_ksy",
    "O04_loww1x_ksx_only",
    "O05_loww1x_relaxedclip",
    "O06_loww1x_rms_tightclip",
    "O07_now1x_prior_ramp",
    "O08_loww1x_inverse_ramp_mmdxy",
    "O09_now1x_corrspearman_xy",
    "O10_now1x_corrpearson_xy",
]

PRIORITY_TOP3_POLICY_ORDER = [
    "O01_now1x_mmdheavy",
    "O03_now1x_ksx_ksy",
    "O09_now1x_corrspearman_xy",
]

PRIORITY_TOP5_POLICY_ORDER = PRIORITY_TOP3_POLICY_ORDER + [
    "O05_loww1x_relaxedclip",
    "O07_now1x_prior_ramp",
]
...
FLOWGEN_OFFICIAL_POLICY_SPECS = {
    "O01_now1x_mmdheavy": {
        ...
        "historical_origin": (
            "Reopen family M / variant 1. Inspired by the shuffled broad sweeps where strong MMD regions were "
            "explored more independently from W1. It explicitly turns off W1_x to test whether the c2 base can "
            "improve X realism through kernel matching instead of more W1 pressure."
        ),
    ...
    "O02_now1x_mmdheavy_mmdxy": {
        ...
        "historical_origin": (
            "Reopen family M / variant 2. Same no-W1_x idea, but adding a light MMD_XY term to test whether the "
            "missing signal is joint structure rather than marginal X fit alone."
        ),
        "historical_source_run_ids": [],
    },
    "O03_now1x_ksx_ksy": {
        ...
        "historical_origin": (
            "Reopen family K / variant 1. This is a shape-matching probe derived from the shuffled scripts that used "
            "light KS as an orthogonal control axis. It tests whether the official c2 line needs explicit KS on both "
            "X and Y once W1_x is removed."
        ),
        "historical_source_run_ids": [],
    },
    "O04_loww1x_ksx_only": {
        ...
        "training_overrides": {
            ...
            "use_ks_x": True,
            "ks_x_weight": 0.012,
            "ks_grid_points_x": 112,
            "ks_tau_x": 0.040,
            "use_ks_y": False,
        },
        "historical_origin": (
            "Reopen family K / variant 2. A low-W1 bridge that keeps some W1_x alive but pushes the new orthogonal "
            "signal into KS_x only, to see whether X needs direct shape control without extra Y anchoring."
        ),
        "historical_source_run_ids": [],
    },
    "O05_loww1x_relaxedclip": {
        ...
        "training_overrides": {
            ...
            "mmd_x_weight": 0.58,
            "mmd_y_weight": 1.50,
            "w1_x_softclip_s": 1.70,
            "w1_y_softclip_s": 1.70,
            "w1_x_clip_perdim": 3.0,
            "w1_y_clip_perdim": 3.0,
            "w1_x_agg_softcap": 3.0,
            "w1_y_agg_softcap": 3.0,
        },
        "historical_origin": (
            "Reopen family C / variant 1. Historical shuffled searches explored clipping as a real axis; here we use "
            "a relaxed clip and softcap regime to test whether the current c2 line is over-constraining a small set "
            "of problematic X dimensions."
        ),
        "historical_source_run_ids": [],
    },
    "O06_loww1x_rms_tightclip": {
        ...
        "historical_origin": (
            "Reopen family C / variant 2. Same low-W1 corridor, but switching to RMS-normalized W1 and a tighter "
            "clip/softcap to test whether the official c2 regime needs a very different per-dimension geometry."
        ),
        "historical_source_run_ids": [],
    },
    "O07_now1x_prior_ramp": {
        ...
        "training_overrides": {
            ...
            "realism_warmup_epochs": 10,
            "realism_ramp_epochs": 20,
        },
        "historical_origin": (
            "Reopen family R / variant 1. Inspired by the shuffled scripts that treated schedule and class weighting "
            "as first-order axes. This asks whether the official c2 branch is being constrained too early and would "
            "benefit from a softer realism activation with prior-aware class balance."
        ),
        "historical_source_run_ids": [],
    },
    "O08_loww1x_inverse_ramp_mmdxy": {
        ...
        "training_overrides": {
            ...
            "realism_warmup_epochs": 15,
            "realism_ramp_epochs": 30,
        },
        "historical_origin": (
            "Reopen family R / variant 2. A more imbalanced and slower schedule variant that couples inverse class "
            "weighting with light joint-XY pressure, specifically to probe whether minority classes are blocking the "
            "official c2 improvements we expected from the base."
        ),
        "historical_source_run_ids": [],
    },
    "O09_now1x_corrspearman_xy": {
        ...
        "training_overrides": {
            ...
            "use_mmd_xy": True,
            "mmd_xy_weight": 0.02,
            "use_corr_xy_spearman": True,
            "corr_xy_spearman_weight": 0.02,
            "corr_xy_tau": 0.04,
        },
        "historical_origin": (
            "Reopen family J / variant 1. This introduces an actually new axis for the official line: monotonic "
            "joint X-Y correlation control. The shuffled sweeps barely used this lane, and our current analysis says "
            "the frontier is too concentrated away from it."
        ),
        "historical_source_run_ids": [],
    },
    "O10_now1x_corrpearson_xy": {
        ...
        "training_overrides": {
            ...
            "use_mmd_xy": True,
            "mmd_xy_weight": 0.02,
            "use_corr_xy_pearson": True,
            "corr_xy_pearson_weight": 0.015,
        },
        "historical_origin": (
            "Reopen family J / variant 2. Same joint-structure hypothesis as O09, but with linear correlation "
            "pressure instead of rank correlation, to keep this family orthogonal even inside itself."
        ),
        "historical_source_run_ids": [],
    },
}
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Official FlowGen reopen-experimental finetune runner. "
            "It reopens only the c2 base with a deliberately orthogonal 10-policy "
            "search space derived from the current official + train-only analysis."
        )
    )
...
def _policy_slug_for_run_id(policy_id: str, base_token: str) -> str:
    slug = policy_id.lower()
    return slug


def _run_id_for(base: BaseContext, policy: OfficialPolicy, run_seed: int) -> str:
    policy_slug = _policy_slug_for_run_id(policy.policy_id, base.token)
    return f"flowgen_tpv1_{base.token}_reopenexp_{policy_slug}_seed{int(run_seed)}_v1"
...
def _build_config_payload(*, base: BaseContext, policy: OfficialPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed": int(run_seed),
        "official_training": {
            "mode": "flowgen_official_reopen_experimental",
            "policy_set_id": "flowgen_official_reopen_experimental_v1",
...
def _build_evaluation_context(*, base: BaseContext, policy: OfficialPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_set_id": f"flowgen_official_reopen_exp_{base.token}_seed{int(run_seed)}",
        ...
        "run_level_axes": {
            "phase": "official_reopen_experimental",
...
def _entries_for_policy_ids(entries: list[PlanEntry], policy_ids: list[str]) -> list[PlanEntry]:
    order = {policy_id: idx for idx, policy_id in enumerate(policy_ids)}
    selected = [entry for entry in entries if entry.policy_id in order]
    selected.sort(key=lambda entry: (order[entry.policy_id], entry.run_id))
    return selected
...
def _print_plan_summary(
    *,
    bases: list[BaseContext],
    policies: list[OfficialPolicy],
    canonical_entries: list[PlanEntry],
    entries: list[PlanEntry],
    summary_paths: dict[str, Path],
) -> None:
    counts = _summarize_counts(entries)
    canonical_counts = _summarize_counts(canonical_entries)
    top3_entries = _entries_for_policy_ids(canonical_entries, PRIORITY_TOP3_POLICY_ORDER)
    top5_entries = _entries_for_policy_ids(canonical_entries, PRIORITY_TOP5_POLICY_ORDER)

    print("\n" + "=" * 100)
    print("FlowGen official reopen experimental")
    print("Run seed policy: reuse the official FlowGen base seed")
    print("Condition: type | target: init | skip_phase1=True | allow_test_holdout=False")
    print("Temperature tuning: disabled | dataset materialization: disabled | realism bootstrap: 10/10")
    ...
    print("\nOfficial reopen policy set:")
    for policy in policies:
        source_count = len(policy.historical_source_run_ids)
        allowed_bases = ", ".join(policy.allowed_base_tokens)
        print(
            f"  - {policy.policy_id}: bases={allowed_bases} | signature={policy.policy_signature[:12]} | "
            f"historical_sources={source_count}"
        )
        print(f"    provenance: {policy.historical_origin}")

    print("\nRuntime policy:")
    print("  - runtime does not load discovery artifacts, historical sweeps, or training scripts")
    print("  - provenance remains metadata only; every reopen policy is explicit in this file")

    print("\nCanonical reopen campaign (10 runs):")
    for entry in canonical_entries:
        print(f"  - {entry.run_id}")

    print("\nRecommended mini-batch (3 runs):")
    for entry in top3_entries:
        print(f"  - {entry.run_id}")

    print("\nRecommended mini-batch (5 runs):")
    ...
    print("\nPlan counts:")
    print(f"  - canonical reopen runs: {canonical_counts['planned_total']}")
...
def _resolve_entry_by_run_id(entries: list[PlanEntry], run_id: str) -> PlanEntry:
    ...
    if not matches:
        raise RuntimeError(f"Run id not found in current FlowGen official reopen plan: {run_id}")
    if len(matches) != 1:
        raise RuntimeError(f"Run id appears multiple times in current FlowGen official reopen plan: {run_id}")
    return matches[0]
...
def main() -> int:
    ...
    canonical_bases = [_resolve_base_context(token) for token in sorted(BASE_RUN_IDS)]
    canonical_entries = _build_plan(
    ...
    if not entries:
        raise RuntimeError("No official FlowGen reopen runs remain after filtering.")
    ...
    if args.run_one:
        ...
        print(f"Running isolated FlowGen official reopen run: {entry.run_id}")
        _run_one(
        ...
        print(f"Completed isolated FlowGen official reopen run: {entry.run_id}")
        return 0

    SUMMARY_ROOT.mkdir(parents=True, exist_ok=True)
    campaign_id = f"flowgen_official_reopen_experimental_{_campaign_timestamp()}"
    ...
    plan_payload = {
        ...
        "contract_id": CONTRACT_ID,
        "output_root": str(_run_dir_for("")).rstrip("/"),
        "priority_groups": {
            "top3_policy_ids": list(PRIORITY_TOP3_POLICY_ORDER),
            "top3_run_ids": [entry.run_id for entry in _entries_for_policy_ids(canonical_entries, PRIORITY_TOP3_POLICY_ORDER)],
            "top5_policy_ids": list(PRIORITY_TOP5_POLICY_ORDER),
            "top5_run_ids": [entry.run_id for entry in _entries_for_policy_ids(canonical_entries, PRIORITY_TOP5_POLICY_ORDER)],
        },
        ...
        "canonical_reopen_runs": [asdict(entry) for entry in canonical_entries],
    ...
    print("\n" + "=" * 100)
    print(f"Starting official FlowGen reopen experimental | planned runs={len(entries)}")
    ...
    print("\n" + "=" * 100)
    print("Official FlowGen reopen experimental finished")
