...
CONTRACT_ID = "experimental_flowpre_train_only_top20_priors_v1"
...
TOP20_PRIORS: tuple[PriorSpec, ...] = (
    PriorSpec(
        1,
        "hf256|l3|rq1x3|frq3|lr1e-3|mson|skoff",
        "flowpre_rrmse_tpv1_layers3_seed5678_v1",
        5678,
    ...
    PriorSpec(
        2,
        "hf192|l2|rq1x3|frq3|lr1e-3|mson|skoff",
        "flowprex4_rrmse_tpv1_hf192_l2_rq3_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        1,
    ),
    PriorSpec(
        3,
        "hf256|l4|rq1x5|frq5|lr1e-3|mson|skoff",
    ...
    PriorSpec(
        4,
        "hf192|l3|rq1x6|frq6|lr1e-3|mson|skoff",
    ...
    PriorSpec(
        5,
        "hf256|l2|rq1x3|frq3|lr1e-3|mson|skoff",
        "flowprex3_rrmse_tpv1_fgp_hf256_l2_rq3_lr1e-3_mson_skoff_seed5678_v1",
        5678,
    ...
    PriorSpec(
        6,
        "hf192|l3|rq1x3|frq3|lr1e-3|mson|skoff",
        "flowprex3_rrmse_tpv1_fgh_hf192_l3_rq3_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        21,
    ),
    PriorSpec(
        7,
        "hf256|l4|rq1x8|frq8|lr1e-3|mson|skoff",
        "flowprex4_rrmse_tpv1_hf256_l4_rq8_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        15,
    ),
    PriorSpec(
        8,
        "hf256|l4|rq1x5|frq5|lr1e-3|mson|skon",
    ...
    PriorSpec(
        9,
        "hf128|l4|rq1x3|frq3|lr1e-3|mson|skoff",
        "flowprex3_rrmse_tpv1_fgr_hf128_l4_rq3_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        8,
    ),
    PriorSpec(
        10,
        "hf128|l4|rq1x5|frq5|lr1e-3|mson|skoff",
        "flowprex4_rrmse_tpv1_hf128_l4_rq5_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        9,
    ),
    PriorSpec(
        11,
        "hf256|l4|rq1x6|frq6|lr1e-3|mson|skoff",
    ...
    PriorSpec(
        12,
        "hf256|l2|rq1x5|frq5|lr1e-3|mson|skoff",
        "flowprex4_rrmse_tpv1_hf256_l2_rq5_lr1e-3_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_robust",
        8,
    ),
    PriorSpec(
        13,
        "hf192|l4|rq1x5|frq5|lr1e-3|mson|skoff",
    ...
    PriorSpec(
        14,
        "hf256|l2|rq1x6|frq6|lr1e-3|mson|skoff",
        "flowprex4_rrmse_tpv1_hf256_l2_rq6_lr1e-3_mson_skoff_seed5678_v1",
        5678,
    ...
    PriorSpec(
        15,
        "hf256|l3|rq1x3|frq3|lr1e-4|mson|skoff",
    ...
    PriorSpec(
        16,
        "hf192|l3|rq1x3|frq3|lr1e-4|mson|skoff",
        "flowprex3_rrmse_tpv1_rp_hf192_l3_rq3_lr1e-4_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_hybrid",
        20,
    ),
    PriorSpec(
        17,
        "hf256|l4|rq1x3|frq3|lr1e-4|mson|skoff",
        "flowpre_rrmse_tpv1_lr1e-4_seed9101_v1",
        9101,
        "flowgencandidate_robust",
        5,
    ),
    PriorSpec(
        18,
        "hf192|l4|rq1x6|frq6|lr1e-4|mson|skoff",
        "flowprex2_rrmse_tpv1_hf192_l4_rq6_lr1e-4_mson_skoff_seed5678_v1",
        5678,
        "flowgencandidate_hybrid",
        21,
    ),
    PriorSpec(
        19,
        "hf128|l4|rq1x6|frq6|lr1e-3|msoff|skoff",
        "flowprex3_fair_tpv1_mvn_hf128_l4_rq6_lr1e-3_msoff_skoff_seed5678_v1",
        5678,
        "flowgencandidate_priorfit",
        14,
    ),
    PriorSpec(
        20,
        "hf128|l2|rq1x6|frq6|lr1e-3|msoff|skoff",
        "flowpre_fair_tpv1_layers2_seed1234_v1",
        1234,
        "flowgencandidate_robust",
        24,
    ),
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Run the experimental FlowPre train-only top20 prior campaign. "
            "Base configs are copied from official FlowPre runs and trained with "
    ...
    ap.add_argument("--run-one", type=int, choices=range(1, 21), default=None, help=argparse.SUPPRESS)
    ap.add_argument(
        "--only-rank",
        type=int,
        choices=range(1, 21),
        action="append",
        metavar="N",
        help="Run only one or more shortlist ranks. Defaults to all 20.",
    )
    ap.add_argument(
        "--skip-existing",
        action="store_true",
...
def _source_config_path(spec: PriorSpec) -> Path:
    return OFFICIAL_FLOWPRE_ROOT / spec.source_run_id / f"{spec.source_run_id}.yaml"


def _run_id(spec: PriorSpec) -> str:
    return f"flowpre_trainonly_top20_cfg{spec.rank:02d}_{_safe_cfg_slug(spec.cfg_signature)}_seed{RUN_SEED}_v1"


def _config_path(spec: PriorSpec) -> Path:
    return CONFIG_ROOT / f"{_run_id(spec)}.yaml"


def _run_dir(spec: PriorSpec) -> Path:
    return ROOT / "outputs" / "models" / OUTPUT_NAMESPACE / "flow_pre" / OUTPUT_SUBDIR / _run_id(spec)


def _build_config(base_config: dict) -> dict:
    return copy.deepcopy(base_config)


def _config_for_completion_check(spec: PriorSpec, run_id: str, run_dir: Path) -> dict:
...
def _selected_priors(args: argparse.Namespace) -> tuple[PriorSpec, ...]:
    if args.run_one is not None:
        return tuple(spec for spec in TOP20_PRIORS if spec.rank == int(args.run_one))
    if not args.only_rank:
        return TOP20_PRIORS
    requested = set(args.only_rank)
    return tuple(spec for spec in TOP20_PRIORS if spec.rank in requested)


def _validate_spec(spec: PriorSpec, config: dict) -> None:
    actual = _cfg_signature(config)
    if actual != spec.cfg_signature:
        raise ValueError(
            "Source config signature mismatch for "
            f"{spec.source_run_id}: expected {spec.cfg_signature}, got {actual}"
        )


def _inspect_run(spec: PriorSpec) -> dict:
    run_id = _run_id(spec)
    ...
    if not missing:
        ...
        try:
            ...
            if run_axes.get("shortlist_rank") != spec.rank:
                missing.append("manifest shortlist_rank mismatch")
        except Exception as exc:
...
def _audit_runs(priors: tuple[PriorSpec, ...]) -> list[dict]:
    return [_inspect_run(spec) for spec in priors]
...
def _print_plan_header(priors: tuple[PriorSpec, ...]) -> None:
    print("Experimental FlowPre train-only top20 prior campaign")
    print(f"Runs selected: {len(priors)}")
...
def _print_spec_plan(spec: PriorSpec) -> None:
    run_id = _run_id(spec)
    print("")
    print(f"[{spec.rank:02d}] {run_id}")
    print(f"  cfg_signature: {spec.cfg_signature}")
    print(f"  source config: {_source_config_path(spec)}")
    print(f"  source view/rank: {spec.source_view} #{spec.source_rank}")
    print(f"  source seed: {spec.source_seed}")
    print(f"  run seed: {RUN_SEED}")
    print("  overrides: monitoring_policy='train_only'; experimental namespace/output naming")
    print(f"  derived config: {_config_path(spec)}")
    print(f"  run dir: {_run_dir(spec)}")


def _train_one(spec: PriorSpec, *, device: str, quiet: bool) -> None:
    source_config = _source_config_path(spec)
    base_config = _load_yaml(source_config)
    _validate_spec(spec, base_config)
    ...
    _print_spec_plan(spec)
    cfg = _build_config(base_config)
    ...
    try:
        ...
        model = train_flowpre_pipeline(
            ...
            evaluation_context=_evaluation_context(spec),
...
def _run_child(spec: PriorSpec, *, device: str, quiet: bool) -> None:
    cmd = [
        sys.executable,
...
def main() -> None:
    args = _parse_args()
    priors = _selected_priors(args)

    if args.run_one is not None:
        if not priors:
            raise RuntimeError(f"Unknown run-one rank: {args.run_one}")
        _train_one(priors[0], device=args.device, quiet=args.quiet)
        return

    _print_plan_header(priors)
    for spec in priors:
        source_config = _source_config_path(spec)
        base_config = _load_yaml(source_config)
        _validate_spec(spec, base_config)
    ...
    initial_audit = _audit_runs(priors)
    ...
    post_cleanup_audit = _audit_runs(priors)
    _print_audit("State after cleanup", post_cleanup_audit)
    pending_specs = [spec for spec in priors if _inspect_run(spec)["status"] != "complete"]
    ...
    final_audit = _audit_runs(priors)
