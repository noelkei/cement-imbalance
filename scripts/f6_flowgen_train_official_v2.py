...
CONTRACT_ID = "f6_flowgen_official_train_v2"
...
PREFERRED_POLICY_ORDER = [
    "H01_bridge300_lowmmd_c2",
    "H02_bridge500_lowmmd_c2",
    "H03_bridge600_lowmmd_c2",
    "H04_bridge500_lowmmd_c1",
    "F01_e33_lowmmd_c2",
    "F02_e34_lowmmd_c2",
    "F03_e35_lowmmd_c2",
    "K01_e36_ksy_c2",
...
FLOWGEN_OFFICIAL_POLICY_SPECS = {
    "H01_bridge300_lowmmd_c2": {
        ...
        "historical_origin": (
            "New explicit official v2 bridge policy. It fills the large unexplored gap between the low-W1 "
            "A45-like regime and the high-W1 E-family regime, while keeping the lower-MMD prior that behaved "
            "well in the current official runs."
        ),
    ...
    "H02_bridge500_lowmmd_c2": {
        ...
        "historical_origin": (
            "New explicit official v2 bridge policy. It probes a mid-strength realism regime that was absent "
            "from the official v1 campaign and was suggested by the tension between classic and banded/blended."
        ),
        "historical_source_run_ids": [],
    },
    "H03_bridge600_lowmmd_c2": {
        ...
        "historical_origin": (
            "New explicit official v2 bridge policy. It sits just below the E32-like official prior to test "
            "whether the c2 base prefers a slightly softer realism push."
        ),
        "historical_source_run_ids": [],
    },
    "H04_bridge500_lowmmd_c1": {
        ...
        "historical_origin": (
            "New explicit official v2 bridge policy for the c1 base. It tests whether candidate_1 benefits from "
            "an intermediate realism regime between the A45-like and E18-like operating points."
        ),
        "historical_source_run_ids": [],
    },
    "F01_e33_lowmmd_c2": {
        ...
        "historical_origin": (
            "Explicit official v2 frontier extension derived from the historical E33 neighborhood. It keeps the "
            "lower-MMD regime and extends the current c2 frontier between E32 and E18."
        ),
        "historical_source_run_ids": [],
    },
    "F02_e34_lowmmd_c2": {
        ...
        "historical_origin": (
            "Explicit official v2 frontier extension derived from the historical E34 neighborhood. It is the "
            "main high-realism c2 probe beyond the official v1 shortlist."
        ),
        "historical_source_run_ids": [],
    },
    "F03_e35_lowmmd_c2": {
        ...
        "historical_origin": (
            "Explicit official v2 frontier extension derived from the historical E35 neighborhood. It tests the "
            "upper edge of the high-W1 regime on the stronger c2 base."
        ),
        "historical_source_run_ids": [],
    },
    "K01_e36_ksy_c2": {
        ...
        "historical_origin": (
            "Explicit official v2 KS-Y probe derived from the historical E36 idea. It keeps the strong E-family "
            "realism pressure and turns on a light KS-Y term that the official v1 campaign never used."
        ),
        "historical_source_run_ids": [],
    },
    "K02_e37_ksy_c1": {
        ...
        "historical_origin": (
            "Explicit official v2 KS-Y probe derived from the historical E37 idea, targeted at the c1 base "
            "because c1_E18 already looked robust under the official v1 evaluation."
        ),
        "historical_source_run_ids": [],
    },
    "S01_e38_softclip_c2": {
        ...
        "training_overrides": {
            ...
            "w1_x_softclip_s": 1.5,
            "w1_y_softclip_s": 1.5,
            "w1_x_clip_perdim": 2.5,
            "w1_y_clip_perdim": 2.5,
        },
        "historical_origin": (
            "Explicit official v2 W1-stability probe derived from the historical E38 idea. It keeps the high-W1 "
            "regime and relaxes the softclip/clip settings to test whether the c2 frontier is limited by W1 saturation."
        ),
        "historical_source_run_ids": [],
    },
    "S02_e38_softclip_c1": {
        ...
        "historical_origin": (
            "Explicit official v2 W1-stability probe derived from the historical E38 idea, targeted at the c1 base "
            "as a rescue attempt for the high-W1 regime."
        ),
        "historical_source_run_ids": [],
    },
    "J01_e18_mmdxy_light_c2": {
        ...
        "historical_origin": (
            "New explicit official v2 joint-XY realism probe. It starts from the robust E18-like c2 regime and "
            "adds a light MMD_XY term, which was not exercised in the official v1 campaign."
        ),
        "historical_source_run_ids": [],
    },
}
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "Canonical official FlowGen trainer v2 for the current base-trained phase. "
            "It starts from the two official FlowGen bases and applies the explicit "
            "official v2 exploration policy set derived from the current official analysis."
        )
    )
...
def _run_id_for(base: BaseContext, policy: OfficialPolicy, run_seed: int) -> str:
    ...
    return f"flowgen_tpv1_{base.token}_train_{policy_slug}_seed{int(run_seed)}_v2"
...
def _build_config_payload(*, base: BaseContext, policy: OfficialPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed": int(run_seed),
        "official_training": {
            "mode": "flowgen_official_base_finetune_v2",
            "policy_set_id": "flowgen_official_v2_exploration",
...
def _build_evaluation_context(*, base: BaseContext, policy: OfficialPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_set_id": f"f6_flowgen_official_v2_{base.token}_seed{int(run_seed)}",
...
def _print_plan_summary(
    *,
    ...
    print("\n" + "=" * 100)
    print("FlowGen official training campaign v2")
    ...
    print("  - history remains only as provenance for explicit current policies")
...
def _resolve_entry_by_run_id(entries: list[PlanEntry], run_id: str) -> PlanEntry:
    ...
    if not matches:
        raise RuntimeError(f"Run id not found in current FlowGen official v2 plan: {run_id}")
    if len(matches) != 1:
        raise RuntimeError(f"Run id appears multiple times in current FlowGen official v2 plan: {run_id}")
    return matches[0]
...
def main() -> int:
    ...
    if not entries:
        raise RuntimeError("No official FlowGen v2 runs remain after filtering.")
    ...
    if args.run_one:
        ...
        print(f"Running isolated FlowGen official v2 run: {entry.run_id}")
        _run_one(
        ...
        print(f"Completed isolated FlowGen official v2 run: {entry.run_id}")
        return 0

    SUMMARY_ROOT.mkdir(parents=True, exist_ok=True)
    campaign_id = f"flowgen_train_official_v2_tpv1_base_seeds_{_campaign_timestamp()}"
    ...
    print("\n" + "=" * 100)
    print(f"Starting official FlowGen v2 training | planned runs={len(entries)}")
    ...
    print("\n" + "=" * 100)
    print("Official FlowGen v2 training finished")
