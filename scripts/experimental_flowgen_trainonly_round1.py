...
OUTPUT_SUBDIR_ROOT = "round1"
MODEL_FAMILY = "flowgen"
CONTRACT_ID = "flowgen_trainonly_round1_v1"
OBJECTIVE_METRIC_ID = "flowgen_trainonly_realism_round1"
...
SUMMARY_ROOT = FLOWGEN_TRAINONLY_ROOT / "campaign_summaries"
...
PREFERRED_POLICY_ORDER = [
    "E03",
    "E32",
    "H02_bridge500_lowmmd",
    "H03_bridge600_lowmmd",
    "F02_e34_lowmmd",
    "F03_e35_lowmmd",
    "K01_e36_ksy",
    "J01_e18_mmdxy_light",
...
FLOWGEN_TRAINONLY_POLICY_SPECS = {
    "E03": {
        ...
        "historical_origin": (
            "Official winner anchor. Final post-reseed best compromise in the canonical line; reused here "
            "as the main control anchor rather than as an assumed train-only winner."
        ),
    ...
    "E32": {
        ...
        "historical_origin": (
            "Canonical low-MMD anchor from the official line; kept here as a cleaner frontier reference than E18."
        ),
        "historical_source_run_ids": [
            "flowgen_tpv1_c2_train_e32_seed2468_v1",
        ],
    },
    "H02_bridge500_lowmmd": {
        "training_overrides": {
        ...
        "historical_origin": (
            "Official bridge regime that won the train-strong objective post-reseed but lost the official closure "
            "because of temporal fragility; promoted here because train-only cares more about fitting train."
        ),
        "historical_source_run_ids": [
    ...
    "H03_bridge600_lowmmd": {
        ...
        "historical_origin": (
            "Official bridge extension above H02, used here to close the train-only gradient between the bridge "
            "regime and the low-MMD frontier."
        ),
        "historical_source_run_ids": [
            "flowgen_tpv1_c2_train_h03_bridge600_lowmmd_seed2468_v2",
        ],
    },
    "F02_e34_lowmmd": {
        "training_overrides": {
        ...
        "historical_origin": (
            "Balanced low-MMD frontier point from the official c2 exploration; strong candidate for train-only "
            "because it pushes fit without relying on the pure official winner rationale."
        ),
        "historical_source_run_ids": [
    ...
    "F03_e35_lowmmd": {
        ...
        "historical_origin": (
            "Aggressive low-MMD frontier point from the official c2 exploration; kept to test whether train-only "
            "benefits from a harder push than the official temporal line could safely accept."
        ),
        "historical_source_run_ids": [
            "flowgen_tpv1_c2_train_f03_e35_lowmmd_seed2468_v2",
        ],
    },
    "K01_e36_ksy": {
        "training_overrides": {
        ...
        "historical_origin": (
            "Official KS-Y probe that was consistently strong on train-facing objectives; promoted here because "
            "target-shape fidelity is more important in the train-only branch."
        ),
        "historical_source_run_ids": [
    ...
    "J01_e18_mmdxy_light": {
        ...
        "historical_origin": (
            "Official light MMD-XY probe; promoted here because preserving joint X-y structure matters more in "
            "train-only downstream balancing than it did in the temporal official closure."
        ),
        "historical_source_run_ids": [
    ...
    "S01_e38_softclip": {
        ...
        "historical_origin": (
            "Former operational winner before post-reseed aggregation; kept here as the main clipping/stability probe "
            "for a stronger train-only regime."
        ),
        "historical_source_run_ids": [
    ...
    "K02_f02_ksy_lowmmd": {
        ...
        "historical_origin": (
            "Official combination probe that layers KS-Y onto the balanced low-MMD frontier; promoted here because "
            "it is the cleanest test of whether train-only benefits from both stronger fit and tighter target shape."
        ),
        "historical_source_run_ids": [
            "flowgen_tpv1_c2_train_k02_f02_ksy_lowmmd_seed2468_v3",
        ],
    },
}
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "FlowGen train-only finetune round 1. Runs the selected 10 policies over the two "
            "promoted train-only bases."
        )
    )
...
def _run_id_for(base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> str:
    ...
    return f"flowgen_trainonly_tpv1_{token}_train_{policy_slug}_seed{int(run_seed)}_v1"
...
def _build_config_payload(*, base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed": int(run_seed),
        "trainonly_training": {
            "mode": "flowgen_trainonly_round1_finetune",
            "policy_set_id": "flowgen_trainonly_round1_v1",
...
def _build_evaluation_context(*, base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_set_id": f"{base.token}_round1_seed{int(run_seed)}",
        ...
        "run_level_axes": {
            "phase": "trainonly_round1_finetune",
...
def _print_plan_summary(
    *,
    ...
    print("\n" + "=" * 100)
    print("FlowGen train-only finetune round 1")
...
def _resolve_entry_by_run_id(entries: list[PlanEntry], run_id: str) -> PlanEntry:
    ...
    if not matches:
        raise RuntimeError(f"Run id not found in current FlowGen train-only round1 plan: {run_id}")
    if len(matches) != 1:
        raise RuntimeError(f"Run id appears multiple times in current FlowGen train-only round1 plan: {run_id}")
    return matches[0]
...
def main() -> int:
    ...
    if not entries:
        raise RuntimeError("No FlowGen train-only round1 runs remain after filtering.")
    ...
    if args.run_one:
        ...
        print(f"Running isolated FlowGen train-only round1 run: {entry.run_id}")
        _run_one(
        ...
        print(f"Completed isolated FlowGen train-only round1 run: {entry.run_id}")
        return 0

    SUMMARY_ROOT.mkdir(parents=True, exist_ok=True)
    campaign_id = f"flowgen_trainonly_round1_{_campaign_timestamp()}"
    ...
    print("\n" + "=" * 100)
    print(f"Starting FlowGen train-only round1 | planned runs={len(entries)}")
    ...
    print("\n" + "=" * 100)
    print("FlowGen train-only round1 finished")
