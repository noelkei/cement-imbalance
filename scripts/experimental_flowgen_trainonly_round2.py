...
OUTPUT_SUBDIR_ROOT = "round2"
MODEL_FAMILY = "flowgen"
CONTRACT_ID = "flowgen_trainonly_round2_v1"
OBJECTIVE_METRIC_ID = "flowgen_trainonly_realism_round2"
...
SUMMARY_ROOT = FLOWGEN_TRAINONLY_ROOT / "campaign_summaries" / "round2"
...
PREFERRED_POLICY_ORDER = [
    "T01_now1x_mmdheavy",
    "T02_now1x_mmdheavy_mmdxy",
    "T03_now1x_ksx_ksy",
    "T04_loww1x_ksx_only",
    "T05_loww1x_relaxedclip",
    "T06_loww1x_rms_tightclip",
    "T07_now1x_prior_ramp",
    "T08_loww1x_inverse_ramp_mmdxy",
    "T09_now1x_corrspearman_xy",
    "T10_now1x_corrpearson_xy",
]
...
FLOWGEN_TRAINONLY_POLICY_SPECS = {
    "T01_now1x_mmdheavy": {
        ...
        "historical_origin": (
            "Round2 family M / variant 1. Inspired by the early shuffled broad sweeps where MMD was explored away "
            "from the final official frontier. It explicitly removes W1_x to test whether the train-only bases can "
            "improve X through kernel matching instead of more W1 pressure."
        ),
    ...
    "T02_now1x_mmdheavy_mmdxy": {
        ...
        "historical_origin": (
            "Round2 family M / variant 2. Same no-W1_x direction as T01, but adding a light MMD_XY term to test "
            "whether train-only mainly needs better joint structure once X is no longer overdriven by W1."
        ),
        "historical_source_run_ids": [],
    },
    "T03_now1x_ksx_ksy": {
        ...
        "historical_origin": (
            "Round2 family K / variant 1. A real shape-control probe derived from the historical shuffled searches, "
            "now applied to train-only where we care more about direct fit to train than temporal caution."
        ),
        "historical_source_run_ids": [],
    },
    "T04_loww1x_ksx_only": {
        ...
        "historical_origin": (
            "Round2 family K / variant 2. Keeps a small amount of W1_x alive but moves the new orthogonal signal "
            "into KS_x only, to test whether the train-only pain is mostly concentrated in X shape rather than Y."
        ),
        "historical_source_run_ids": [],
    },
    "T05_loww1x_relaxedclip": {
        ...
        "historical_origin": (
            "Round2 family C / variant 1. Relaxes clipping and the aggregate softcap to see whether the train-only "
            "bases are being held back by a handful of offending X dimensions."
        ),
        "historical_source_run_ids": [],
    },
    "T06_loww1x_rms_tightclip": {
        ...
        "historical_origin": (
            "Round2 family C / variant 2. Keeps the same low-W1 corridor as T05 but flips to RMS-normalized W1 and "
            "a much tighter clip, making this a genuinely different geometry probe rather than a nearby variant."
        ),
        "historical_source_run_ids": [],
    },
    "T07_now1x_prior_ramp": {
        ...
        "historical_origin": (
            "Round2 family R / variant 1. Train-only is imbalanced by construction, so this explicitly tests whether "
            "a prior-aware class regime and a softer realism activation help the base move in a different direction."
        ),
        "historical_source_run_ids": [],
    },
    "T08_loww1x_inverse_ramp_mmdxy": {
        ...
        "historical_origin": (
            "Round2 family R / variant 2. This is the minority-focused sibling of T07: inverse class weighting, slow "
            "schedule, and a light joint-XY term, specifically to test minority-class pressure in train-only."
        ),
        "historical_source_run_ids": [],
    },
    "T09_now1x_corrspearman_xy": {
        ...
        "historical_origin": (
            "Round2 family J / variant 1. A genuinely new lane for train-only: monotonic joint X-Y correlation "
            "control. We barely touched this axis before, and the analysis says the current frontier is too narrow."
        ),
        "historical_source_run_ids": [],
    },
    "T10_now1x_corrpearson_xy": {
        ...
        "historical_origin": (
            "Round2 family J / variant 2. Same joint-structure hypothesis as T09, but with linear correlation "
            "pressure instead of rank correlation, to keep the family genuinely orthogonal inside itself."
        ),
        "historical_source_run_ids": [],
    },
}
...
def _parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description=(
            "FlowGen train-only finetune round 2. Runs a new orthogonal 10-policy search "
            "over the two promoted train-only bases."
        )
    )
...
def _run_id_for(base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> str:
    ...
    return f"flowgen_trainonly_tpv1_{token}_round2_{policy_slug}_seed{int(run_seed)}_v1"
...
def _build_config_payload(*, base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed": int(run_seed),
        "trainonly_training": {
            "mode": "flowgen_trainonly_round2_finetune",
            "policy_set_id": "flowgen_trainonly_round2_v1",
...
def _build_evaluation_context(*, base: BaseContext, policy: TrainOnlyPolicy, run_seed: int) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_set_id": f"{base.token}_round2_seed{int(run_seed)}",
        ...
        "run_level_axes": {
            "phase": "trainonly_round2_finetune",
...
def _print_plan_summary(
    *,
    ...
    print("\n" + "=" * 100)
    print("FlowGen train-only finetune round 2")
    ...
    print("Realism bootstrap: 10/10 | policy goal: orthogonal round2 exploration")
...
def _resolve_entry_by_run_id(entries: list[PlanEntry], run_id: str) -> PlanEntry:
    ...
    if not matches:
        raise RuntimeError(f"Run id not found in current FlowGen train-only round2 plan: {run_id}")
    if len(matches) != 1:
        raise RuntimeError(f"Run id appears multiple times in current FlowGen train-only round2 plan: {run_id}")
    return matches[0]
...
def main() -> int:
    ...
    if not entries:
        raise RuntimeError("No FlowGen train-only round2 runs remain after filtering.")
    ...
    if args.run_one:
        ...
        print(f"Running isolated FlowGen train-only round2 run: {entry.run_id}")
        _run_one(
        ...
        print(f"Completed isolated FlowGen train-only round2 run: {entry.run_id}")
        return 0

    SUMMARY_ROOT.mkdir(parents=True, exist_ok=True)
    campaign_id = f"flowgen_trainonly_round2_{_campaign_timestamp()}"
    ...
    print("\n" + "=" * 100)
    print(f"Starting FlowGen train-only round2 | planned runs={len(entries)}")
    ...
    print("\n" + "=" * 100)
    print("FlowGen train-only round2 finished")
